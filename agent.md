...
- **App Router + Client Page**: `app/page.tsx` is a large client component that manages the AI workflow, keeps conversation context, and drives UI state (tabs, sandbox tree, preview iframe).
...
- **Progress & Preview**: `components/CodeApplicationProgress.tsx` renders streaming status, and `components/SandboxPreview.tsx` (not shown in tabs but present) handles iframe previewing of the sandbox build.
...
- `lib/context-selector.ts` combines manifest metadata and heuristics (e.g., always include `App.jsx`, Tailwind config) to prime the model with precise instructions and constraints (“surgical edits”, key file list).
...
- **Framer Motion/Tailwind**: used on the client for rich animations and theming but also signal which packages must be available inside the sandbox starter template.
...
- Input/output strings show encoding issues (garbled Cyrillic) in `.env.example`, `README.md`, and `layout.tsx` metadata—clean up before production.
